...
AdminZkClient.createTopic()方法主要做两件事，一是调用AdminUtils.createTopic()方法（后面详细分析），二是将topic相关信息写入zk中（不做展开）。AdminUtils.createTopic()方法中实现了自动分配副本的功能， 首先从ZooKeeper中获取Broker的信息并封装成BrokerMetadata集合， BrokerMetadata中只包含BrokerId和rack（机架） 信息。 之后调用AdminUtils.assignReplicasToBrokers()方法， 它会根据上述Broker信息和命令参数进行自动分配， 其中对于不需要机架感知的分配调用AdminUtils. assignReplicasToBrokersRackUnaware()方法进行处理， 对于需要机架感知的分配调用AdminUtils. assignReplicasToBrokersRackAware()方法进行处理。 最后使用AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK()方法将分配结果写入到ZooKeeper中。 

assignReplicasToBrokersRackUnaware()方法的代码如下（对于创建topic而言，fixedStartIndex和startPartitionId均为-1）：  

```scala
  private def assignReplicasToBrokersRackUnaware(nPartitions: Int,          // 分区数
                                                 replicationFactor: Int,    // 副本引子
                                                 brokerList:Seq[Int],       // broker列表
                                                 fixedStartIndex:Int,       // 第一个副本分配到哪个broker
                                                 startPartitionId: Int):    // 从哪个分区开始分配
	Map[Int,Seq[Int]] = {
...
下面通过一个示例对assignReplicasToBrokersRackUnaware()方法的分配过程进行详细说明， 假设集群中有5个broker， 10个分区0~9， 每个分区分配3个副本， 无机架信息， 并且fixedStartIndex和startPartitionId均为-1， startIndex随机得到的值为2， nextReplicaShift随机得到的值为1。 首先分区0的“优先副本”分配的firstReplicaIndex为(0 + 2)%4=2，即分配到Broker2上， 分配其他副本通过replicaIndex()方法实现：  
...
创建的topic信息会写入zk的/brokers/topics路径下，控制器（todo）会在该路径设置TopicChangeHandler（todo），用来处理主题增减的变化。
...
3. 查询topic
   ...
   ```shell
   # 查询名为test_1的topic的详情（会显示分区、leader、副本、ISR信息），--topic可以没有、单个topic或topic列表
   kafka-topics.sh --bootstrap-server 192.168.181.236:9092 --describe --topic test_1
   ```

   ```shell
   # 不常用参数
   --topics-with-overrides：和--describe一起可以展示与集群配置不一样的主题
   --under-replicated-partitions：和--describe一起可以展示副本数不足的分区
   --unavailable-partitions：和--describe一起可以展示没有leader副本的分区
   ```
...
broker节点上下线后，该节点上的leader副本会转换为follower副本，进而造成整个集群的leader副本分布不均衡，导致负载不均衡。该优先副本选举脚本作用就是将优先副本设置为leader副本使leader副本分布均衡。
...
kafka逻辑中默认开启了leader副本的均衡，开关为auto.leader.rebalance.enable（默认true），周期为leader.imbalance.check.interval.seconds（默认300s），触发阈值为leader.imbalance.per.broker.percentage（默认10%，会计算每个broker节点的分区不平衡率，broker不平衡率=非优先副本为leader的分区个数/分区总数）。

在实际生产环境中，auto.leader.rebalance.enable建议设置为false，因为可能引起客户端一定时间的阻塞，可以设置相关的告警，在合适的时机手动执行分区均衡。
...
kafka-reassign-partitions脚本的主要功能有三个： 一是生成副本迁移的方案； 二是将副本迁移方案写入到ZooKeeper中， 由PartitionsReassignedListener处理； 三是检测指定分区的副本迁移是否完成。这三个步骤一般都是顺序执行，一起配合完成副本迁移工作。
...
ReassignPartitionsCommand.generateAssignment()方法负责输出当前副本的分配情况，并生成副本迁移方案。它会读取topics-to-move-json-file参数和broker-list参数，然后调用AdminUtils.assignReplicasToBrokers()方法生成分配方案（该方法详见kafka-topics.sh节的介绍，fixedStartIndex和startPartitionId均为默认值-1）。

ReassignPartitionsCommand.executeAssignment()方法负责将副本迁移方案写入zk中，它会读取reassignment-json-file参数获得副本迁移方案，并将方案写入zk中。另外还会读取--throttle和--replica-alter-log-dirs-throttle，分别表示副本在broker间和broker内迁移的限流。当迁移的目标信息写入到“/admin/reassign_partitions”路径后， 会触发PartitionsReassignedListener监听器（详细请参考“kafka中的zk-listener”一文）。基本原理是先通过控制器为每个分区添加新副本（增加副本引子），新的副本将从分区的leader副本那里复制所有的数据。在复制完成之后，控制器将旧副本从副本清单里移除（修复为原先的副本因子数）。**注意，重分配的过程中要确保有足够的空间**。重分配后可能存在leader副本不均衡的问题，需要执行一遍优先副本选举脚本kafka-preferred-replica-election.sh。
...
在上面的命令中，还可以加上--replica-alter-log-dirs-throttle参数来限制broker内部的传输流量。

**可以多次执行上面的命令，来改变限制的速度，对迁移无影响。**
...
2. 缩broker：集群需要回收物理机，按照上面的步骤执行即可（**tips：如果设置了多副本，可以先将待回收节点上的broker下线。有两个好处：一是可以立即回收物理机；二是leader副本会从这台broker转移到其它broker，减小了broker间的复制流量**）
...
```
throughput: 限制每秒最多多少条消息（核心设计与设计原理书中说代表限制每秒的吞吐，但实际看了源码是限制条数）
...
ConsumerPerformance底层是通过java客户端的KafkaConsumer完成真正的消费动作的。
...
书本4.1.5节（topic级别配置的修改）和4.3.3节（broker级别配置的修改）
...
```
 sh kafka-broker-api-versions.sh --bootstrap-server 192.168.33.150:9092
```
