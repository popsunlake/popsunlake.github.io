'use strict';
const VERSION='shandong-trip-v1-5-20260906-a';
...
self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;const u=new URL(e.request.url);if(u.origin!==location.origin||!u.pathname.includes('/shandong-trip-2026/'))return;e.respondWith(caches.match(e.request).then(c=>c||fetch(e.request).then(r=>{const cp=r.clone();caches.open(VERSION).then(cache=>cache.put(e.request,cp));return r}).catch(()=>c)))});
